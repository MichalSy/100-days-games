(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`2026-06-11`;function t(e,t){let n=new Date(e);return n.setUTCDate(n.getUTCDate()+t),n}function n(e){return e.toISOString().slice(0,10)}var r=Array.from({length:100},(r,i)=>{let a=i+1;return{day:a,date:n(t(new Date(`${e}T00:00:00Z`),i)),status:`upcoming`,title:`Day ${a.toString().padStart(3,`0`)}`,description:`Not generated yet. The nightly agent will create exactly one detailed prompt and one self-contained game for this day.`,generationDuration:void 0}});r[0]={day:1,date:e,status:`generated`,title:`Lumen Lanes`,slug:`lumen-lanes`,genre:[`Arcade puzzle`,`Route planning`,`Reflex strategy`],mode:`2d`,description:`Rotate neon city lane tiles, connect the cyan source to the gold beacon, and launch a courier pulse through five escalating sectors before energy expires.`,objective:`Clear five sectors by building continuous light routes from start node to exit beacon.`,playUrl:`/001/`,promptUrl:`/games/001/prompt.html`,promptExcerpt:`Guide a courier of light through a shifting city grid by rotating lane tiles before energy runs out.`,screenshotUrl:`/games/001/screenshot.png`,generationDuration:`local cron implementation run`,generatedAt:`2026-06-11`};var i=document.querySelector(`#app`);if(!i)throw Error(`Missing #app root`);var a=r.filter(e=>e.status===`generated`).length,o=r.find(e=>e.status===`upcoming`);function s(e){return String(e).padStart(3,`0`)}function c(e){let t=e.status===`generated`,n=e.genre?.length?e.genre.join(` · `):`Awaiting generation`,r=e.mode?e.mode.toUpperCase():`TBD`,i=t&&e.promptUrl?`<a class="card-action ghost" href="${e.promptUrl}">View prompt</a>`:`<span class="card-action disabled">Prompt appears after generation</span>`,a=t&&e.playUrl?`<a class="card-action primary" href="${e.playUrl}">Play</a>`:`<span class="card-action disabled">Locked</span>`,o=e.screenshotUrl?`<img src="${e.screenshotUrl}" alt="Screenshot of ${e.title}" loading="lazy" />`:`<div class="placeholder-orb"><span>${s(e.day)}</span></div>`;return`
    <article class="game-card ${t?`generated`:`upcoming`}">
      <div class="card-media">${o}</div>
      <div class="card-body">
        <div class="card-kicker">
          <span>Day ${s(e.day)}</span>
          <span>${e.date}</span>
        </div>
        <h3>${e.title}</h3>
  ...
  <main>
    <section class="hero" id="experiment">
      ...
      <dl class="stats">
        <div><dt>${a}</dt><dd>generated</dd></div>
        <div><dt>${100-a}</dt><dd>remaining</dd></div>
        <div><dt>${o?`Day ${s(o.day)}`:`Done`}</dt><dd>next target</dd></div>
      </dl>
    </section>
...
`;var l=document.querySelector(`.theme-toggle`);localStorage.getItem(`theme`)===`light`&&(document.documentElement.dataset.theme=`light`),l?.addEventListener(`click`,()=>{let e=document.documentElement.dataset.theme===`light`?`dark`:`light`;document.documentElement.dataset.theme=e,localStorage.setItem(`theme`,e)});